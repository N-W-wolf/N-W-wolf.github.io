...
第二， **在有限状态、有限动作的情形下，如果每次都精确完成策略评估和策略改进，那么策略迭代会在有限步内收敛到最优策略。**<br>
直观上，这是因为可能的确定性策略总数有限，而每次策略改进都会让策略严格变好或保持不变；一旦某一轮改进后策略不再变化，就说明已经不能继续改进，此时得到的就是最优策略。

第三， **它把“求最优策略”分解成了一系列更容易处理的子问题。**<br>
...
策略迭代的优点是逻辑分明，每一轮策略改进都有清楚的依据；缺点是每轮评估可能比较重。<br>
...
因此，策略改进可以理解为一种从贝尔曼方程走向贝尔曼最优方程的过渡动作。<br>
